import { AlertCircle, AlertTriangle, CheckCircle2, Download, FileInput, FileText, X } from "lucide-react";
    ...
    <section ref={sectionRef} className="ws-compact-surface ws-panel-padding scroll-mt-24">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-4">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[var(--accent-soft)] text-[var(--accent)]"><FileInput size={20} /></span>
        ...
        <button type="button" onClick={downloadTemplate} className="ws-button-secondary flex h-10 shrink-0 items-center justify-center gap-2 rounded-full px-4 text-xs font-semibold"><Download size={15} />{t("contentImport.downloadTemplate")}</button>
      </div>
      ...
      {notice ? <div role="status" className={`mt-4 flex items-center gap-2 rounded-xl px-3 py-2.5 text-xs ${notice.tone === "success" ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-red-500/10 text-red-700 dark:text-red-300"}`}>{notice.tone === "success" ? <CheckCircle2 size={15} /> : <AlertCircle size={15} />}<span>{notice.message}</span></div> : null}

      {drafts.length ? <div className="mt-5 rounded-2xl border border-[var(--border-strong)] bg-[var(--surface-solid)]">
        ...
        <div className="max-h-[26rem] divide-y divide-[var(--border)] overflow-y-auto">
          {drafts.map((draft) => {
            ...
            return <div key={draft.key} className={`grid gap-3 p-4 sm:grid-cols-[auto_minmax(0,1fr)_10rem] sm:items-center ${selected.has(draft.key) ? "" : "opacity-55"}`}>
              ...
              <select value={draft.type} onChange={(event) => patchType(draft.key, event.target.value as EntryType)} className="ws-input h-9 rounded-xl px-3 text-xs">{entryTypes.map((type) => <option key={type} value={type}>{t(`type.${type}`)}</option>)}</select>
            </div>;
          })}
        </div>
