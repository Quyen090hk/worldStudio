    ...
    <aside className="ws-surface overflow-y-auto rounded-[2rem] p-5">
      <div className="space-y-4">
        ...
        <details className="rounded-xl border border-[var(--border)] p-3">
          ...
          <div className="mt-3 space-y-2"><select value={connectionTarget} onChange={(event) => setConnectionTarget(event.target.value)} className="ws-input h-10 w-full rounded-xl px-2 text-xs"><option value="">{t("map.connectMarker")}</option>{mapMarkers.filter((marker) => marker.id !== selected.id).map((marker) => <option key={marker.id} value={marker.id}>{marker.title}</option>)}</select><div className="flex gap-2"><select value={connectionType} onChange={(event) => setConnectionType(event.target.value as ConnectionType)} className="ws-input h-10 min-w-0 flex-1 rounded-xl px-2 text-xs">{MAP_CONNECTION_TYPES.map((type) => <option key={type} value={type}>{t(`map.connection.${type}`)}</option>)}</select><button onClick={connectMarkers} disabled={!connectionTarget} className="ws-button-primary flex h-10 w-10 items-center justify-center rounded-xl"><Link2 size={15} /></button></div>{selectedConnections.map((connection) => { const otherId = connection.fromMarkerId === selected.id ? connection.toMarkerId : connection.fromMarkerId; return <div key={connection.id} className="flex items-center gap-2 rounded-lg bg-[var(--surface-muted)] p-2 text-xs"><Route size={14} /><span className="flex-1 truncate">{t(`map.connection.${connection.type}`)} · {mapMarkers.find((marker) => marker.id === otherId)?.title}</span><button onClick={() => store.deleteConnection(connection.id)}><X size={13} /></button></div>; })}</div>
        </details>
