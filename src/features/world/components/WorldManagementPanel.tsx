  ...
  return (
    <section className="ws-compact-surface ws-panel-padding">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--accent-soft)] text-[var(--accent)]"><LibraryBig size={18} /></span><div><h3 className="ws-display text-2xl font-semibold">{t("worlds.title")}</h3><p className="mt-1 text-xs leading-5 text-[var(--text-faint)]">{t("worlds.description")}</p></div></div>
      </div>
      {notice ? <div role="status" className={`mt-4 flex items-center gap-2 rounded-xl border px-3 py-2.5 text-xs ${notice.tone === "success" ? "border-emerald-500/25 bg-emerald-500/8 text-emerald-700 dark:text-emerald-300" : "border-red-500/25 bg-red-500/8 text-red-700 dark:text-red-300"}`}>{notice.tone === "success" ? <CheckCircle2 size={15} /> : <AlertCircle size={15} />}<span>{notice.message}</span></div> : null}
      {pendingImport ? <div className="mt-4 rounded-2xl border border-[var(--border-strong)] bg-[var(--surface-solid)] p-4">
